...
// import ProfileShowcase from '@/components/ProfileShowcase'; // 3D model paused
...
<div className="relative max-w-7xl mx-auto px-5 sm:px-8 pt-32 pb-20 w-full">
        <div className="grid lg:grid-cols-[1.05fr_0.95fr] gap-8 lg:gap-6 items-center">
            ...
            >
              {STATS.map((s) => (
                <div key={s.label}>
                  <CountUp
                    value={s.value}
                    className="font-display text-[2rem] font-bold text-[var(--text)] leading-none tabular-nums block"
                  />
          ...
          {/* ── Right: interactive 3D profile (paused) ──── */}
          {/* <ProfileShowcase className="w-full h-[320px] sm:h-[440px] lg:h-[560px] animate-fade-up delay-200" /> */}
        </div>
      </div>
    </section>
  );
